...
export default function RecipeList({ onSelectRecipe }) {
  ...
  const handleReaction = (e, id, type) => {
    ...
    const entry = cur[id] || { likes: 0, dislikes: 0, mine: null };
    if (entry.mine === type) {
        ...
        <div key={categoryName} className="category-section">
          ...
          <div className="recipes-grid">
            {categoryRecipes.map((recipe) => {
              const r = reactions[recipe.id] || { likes: 0, dislikes: 0, mine: null };
              return (
                <div
                  key={recipe.id}
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                    <button onClick={(e) => handleReaction(e, recipe.id, 'like')} className="reaction-button">👍 {r.likes || 0}</button>
                    <button onClick={(e) => handleReaction(e, recipe.id, 'dislike')} className="reaction-button">👎 {r.dislikes || 0}</button>
